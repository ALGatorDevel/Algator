...
public class ADECommand {
  ...
  private ADECommandStatus status;
  ...
  public ADECommand(int ID, String commandLine, String outputFile) {
    this.ID          = ID;
    ...
    this.status = ADECommandStatus.NEW;
  }
  ...
  public void start() {
    ...
    if (result instanceof Process) {
      status       = ADECommandStatus.RUNNING;
      statusString = status.toString();
      proces = (Process) result;
    } else {
      status       = ADECommandStatus.ERROR;
      statusString = (String) result;
      proces = null;      
    }
  }

  public ADECommandStatus getStatus() {
    lastTouch = new Date();
    
    if (status.equals(ADECommandStatus.RUNNING) && !proces.isAlive()) 
        status = ADECommandStatus.DONE;
    
    if (!status.equals(ADECommandStatus.ERROR))
      statusString = status.toString();
  ...
  public void stop() {
    ...
    status = ADECommandStatus.STOPPED;
  }
  ...
  public String getOutput() {
    ...
    if (!getStatus().equals(ADECommandStatus.RUNNING)) finalOutputRead=true;
  ...
  public static void main(String[] args) {
    ADECommand command = new ADECommand(1, "Execute BasicSort -dr /Users/Tomaz/ALGATOR_ROOT/data_root -t TestSet1 -a QuickSort -v 2 -e", "output.my");    
    command.start();
    command.stop();
  }
