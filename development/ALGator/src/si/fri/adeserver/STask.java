 ...
 * @author tomaz
 */
public class STask extends Entity implements Comparable<STask> {    
   ...
   */
  public STask(String project, String algorithm, String testset, String mType) {
    this();
  ...
  public final void setTaskStatus(TaskStatus status, String computer) {
    setTaskStatus(status, computer, null);
  }
  public final void setTaskStatus(TaskStatus status, String computer, String msg) {
  ...
  private TaskStatus getTS(String key) {
  ...
  public MeasurementType getMType() {
    ...
    try { 
      mt = MeasurementType.valueOf(((String)getField(STask.ID_MType)).toUpperCase()); 
    } catch (Exception e) {}
    return mt;
  }
  ...
  public String statusMsg() {
    String result = "";
    if (getTaskStatus().equals(TaskStatus.INPROGRESS))
      return ADETools.getTaskStatus(this);
    else if (getTaskStatus().equals(TaskStatus.PENDING))
   ...
   */
  public int compareTo(STask t) {
    try {
      int compare = compare(t);
      if (compare == 0) {
        int tS =   getTaskStatus().equals(TaskStatus.INPROGRESS) ? 0 : (  getTaskStatus().equals(TaskStatus.QUEUED) ? 1 : (  getTaskStatus().equals(TaskStatus.PENDING) ? 2 : 3));
        int oS = t.getTaskStatus().equals(TaskStatus.INPROGRESS) ? 0 : (t.getTaskStatus().equals(TaskStatus.QUEUED) ? 1 : (t.getTaskStatus().equals(TaskStatus.PENDING) ? 2 : 3));
        return oS - tS; 
      } else
        return compare;
