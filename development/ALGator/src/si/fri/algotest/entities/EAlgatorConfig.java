...
public class EAlgatorConfig extends Entity  implements Serializable {
  ...
  public static final String ID_TaskServerPort = "TaskServerPort";     // int

  
  public EAlgatorConfig() {  
   super(ID_ResultParameter, 
	 new String [] {ID_Families, ID_Computers, ID_DBServer, ID_TaskServerName, ID_TaskServerPort});
  ...
  public static String getTaskServerName() {
    EAlgatorConfig config = getConfig();
    if (config != null && config.getString(ID_TaskServerName) != null && !config.getString(ID_TaskServerName).isEmpty())
      return config.getField(ID_TaskServerName);
    return ADEGlobal.DEFAULT_TASK_SERVER_NAME;
  }
  public static int getTaskServerPort() {
    EAlgatorConfig config = getConfig();
    if (config != null && config.getFieldAsInt(ID_TaskServerPort) != 0)
      return config.getFieldAsInt(ID_TaskServerPort);
    return ADEGlobal.DEFAULT_TASK_SERVER_PORT;
  }
